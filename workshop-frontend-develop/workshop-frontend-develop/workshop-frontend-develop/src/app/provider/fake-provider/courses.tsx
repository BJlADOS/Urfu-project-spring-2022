...
export const courses = {
  getCourses: (filters?: { term?: string, roleId?: number, hideClosed?: boolean, hideCompleted?: boolean, showMine?: boolean }): Promise<any> => {
  ...
  getCourseTopic: (topicId: number, courseId: number): Promise<any> => {
    ...
    return new Promise((resolve) => {
      setTimeout(() => resolve({ status: 200, data: fakeCoursesData.find(course => course.id === courseId)?.content.find(topic => topic.id === topicId) }), 1000)
    })
  },
